...
private:
    ...
    // roster.
    Vector3 m_eyePos;
    // Everywhere the player's side is looking from this frame, and how far each
    // of them reaches: their own eye above, then every living squadmate. Sight
    // is shared across a side — what one of them can see, all of them can — and
    // it is no longer the same distance for each, so this list is what the fog is
    ...
    std::vector<Visibility::Eye> m_teamEyes;
