...
- True multiplayer over that seam, LAN-grade today: `infantry_server.exe` hosts the match headless (no D3D on its link line) with AI in every unclaimed slot, and clients join over ENet — commands up, snapshots and events down, sixty a second. A joining player displaces an AI soldier from the emptier side; a leaver's slot goes back to the AI on the same reinforcement clock a death starts. No client prediction yet, so your own soldier answers a round-trip late — imperceptible on localhost and a LAN, and the next milestone on the list
...
- Netcode, the rest of it: client-side prediction & reconciliation for the local soldier, delta-compressed snapshots, fog-based relevance filtering (the anti-wallhack the visibility system already knows how to answer), a connect-to-IP box in the menu, server discovery
