...
- And the name is on the field as well as on the board: under the boots of the soldier driving it, small enough to read past rather than around. Green for your side and red for theirs — the one mark in the arena drawn from the reader's end rather than the same for everyone, because armor answers "which side is that" and has to look identical on both screens, while a name answers "is that one of mine", which is a question about whoever is looking. Only players have one, so a name is what a person looks like from across the map and the AI goes unlabelled by having nothing to label it with — a soldier with words under them is somebody who chose to be there. It obeys the fog exactly, and by construction rather than by agreement: the label is drawn by the same pass that drew the body, from the same position, under the same test, so there is no second opinion about who can be seen and an enemy the arena is hiding has no name to give them away
